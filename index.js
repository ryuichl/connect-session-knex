...
module.exports = function(connect) {
	...
	*/
	...
	function expiredCondition(knex) {
		var condition ='CAST(? as '+timestampTypeName(knex)+') <= expires';
		if(isSqlite3(knex)) { 	// sqlite3 date condition is a special case.
			condition = 'datetime(?) <= datetime(expires)';
		} else if (isOracle(knex)) {
			condition = 'CAST(? as '+timestampTypeName(knex)+') <= "expires"';
		}
		return condition;
	}
	...
	*/
	function getMssqlFastQuery(tablename, sidfieldname) {
		return 'merge ' + tablename + ' as T ' +
			'using (values (?, ?, ?)) as S (' + sidfieldname + ', expires, session) ' +
			'on (T.' + sidfieldname + ' = S.' + sidfieldname + ') ' +
			'when matched then ' +
			'update set expires = S.expires, session = S.session ' +
			'when not matched by target then ' +
			'insert (' + sidfieldname + ', expires, session) values (S.' + sidfieldname + ', S.expires, S.session) ' +
			'output inserted.*;';
	}
	...
	*/
	function dbCleanup(store, interval) {
		return store.ready.then(function () {
			var condition = 'expires < CAST(? as ' + timestampTypeName(store.knex) + ')';
			if(isSqlite3(store.knex)) { 	// sqlite3 date condition is a special case.
				condition = 'datetime(expires) < datetime(?)';
			} else if (isOracle(store.knex)) {
				condition = '"expires" < CAST(? as ' + timestampTypeName(store.knex) + ')';
			}
	...
	* @api public
	*/
	KnexStore.prototype.set = function(sid, session, fn) {
		var self = this;
		...
		if (isSqlite3(self.knex)) {
			...
			return self.ready.then(function () {
				return self.knex.raw(getSqliteFastQuery(self.tablename, self.sidfieldname), [sid, dbDate, session ])
			...
			return self.ready.then(function () {
				return self.knex.raw(getPostgresFastQuery(self.tablename, self.sidfieldname), [sid, dbDate, session ])
				.asCallback(fn);
			});
		} else if (isMySQL(self.knex)) {
			// mysql/mariaDB optimized query
			return self.ready.then(function () {
				return self.knex.raw(getMysqlFastQuery(self.tablename, self.sidfieldname), [sid, dbDate, session ])
				.asCallback(fn);
			});
		} else if (isMSSQL(self.knex)) {
			// mysql/mariaDB optimized query
			return self.ready.then(function () {
				return self.knex.raw(getMssqlFastQuery(self.tablename, self.sidfieldname), [sid, dbDate, session ])
				.asCallback(fn);
			});
		} else {
			return self.ready.then(function () {
	 ...
	 * @public
	 */
	KnexStore.prototype.touch = function(sid, session, fn) {
		if (session && session.cookie && session.cookie.expires) {
			...
			return this.knex(this.tablename)
				...
				.update({
					expires: dateAsISO(this.knex, session.cookie.expires)
				})
				.asCallback(fn);
		}
